...
import {
  ...
  getHistory, addHistoryRecord, deleteHistoryRecord, clearHistoryByUser,
...
export default function App() {
  ...
  const startLevel = (level, retryList = null) => {
    setConfig({ level, modeType: settings.modeType ?? 'tenkey', count: QUESTION_COUNT, retryList })
    setScreen('GAME')
  }
  ...
  const handleFinish = async (res) => {
    ...
    const rec = {
      ...
      isRetry: !!config.retryList,
    }
    await addHistoryRecord(user.id, rec)
    setHistory(prev => [...prev, rec])
    // 通常出題のテンキーで100%ならバッジ獲得
    if (config.modeType === 'tenkey' && accuracy === 100 && !config.retryList && !badges[config.level]) {
  ...
  const counts = {}
  for (const h of history) counts[h.level] = (counts[h.level] ?? 0) + 1
