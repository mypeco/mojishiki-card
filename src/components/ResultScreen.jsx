...
export const ResultScreen = ({ result, modeType, onRetry, onHome, onRetryChallenge, settings }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full bg-indigo-50 animate-pop p-6">
      <div className="text-[7rem] animate-bounce mb-2 select-none filter drop-shadow-md">{stamp}</div>
      <h2 className="text-2xl font-black text-indigo-600 mb-4">{msg}</h2>

      <div className="bg-white rounded-3xl p-6 w-full max-w-xs shadow-lg mb-4">
        <div className="flex justify-between items-end border-b border-indigo-100 pb-2 mb-2">
      ...
      <div className="flex gap-4 w-full max-w-xs">
        ...
        <button onClick={onRetry} className="flex-1 py-4 bg-indigo-400 text-white rounded-2xl font-bold shadow-md active:translate-y-1">もう一度</button>
      </div>
    </div>
  )
}
