...
const COLOR_OPTIONS = [
  { label: 'teal',   class: 'bg-teal-50',    border: 'border-teal-200' },
  { label: 'cyan',   class: 'bg-cyan-50',    border: 'border-cyan-200' },
...
export const ProfileSelect = ({ db, onSelect }) => {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-full p-6 animate-fade-in bg-white">
        ...
        <input type="text" placeholder="名前" value={newName} onChange={e => setNewName(e.target.value)}
          className="w-full max-w-sm text-center text-2xl p-4 rounded-xl border-4 border-teal-200 focus:border-teal-400 outline-none mb-6 shadow-sm" />
          ...
          <div className="grid grid-cols-6 gap-2 max-h-40 overflow-y-auto p-3 bg-white rounded-xl border-2 border-stone-100 shadow-inner">
            {ICON_OPTIONS.map(icon => (
              <button key={icon} onClick={() => setNewIcon(icon)}
                className={`text-3xl p-1 rounded-lg transition-transform ${newIcon === icon ? 'bg-teal-100 ring-4 ring-teal-400 scale-110' : 'hover:bg-stone-50'}`}>
                {icon}
              </button>
            ))}
          </div>
        ...
        <div className="flex gap-4 w-full max-w-sm">
          ...
          <button onClick={handleSave} disabled={!newName.trim()}
            className="flex-1 py-4 rounded-full font-bold text-white bg-teal-500 disabled:opacity-40 active:scale-95 shadow-md transition-transform">
            保存する
          </button>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-full p-6 animate-fade-in bg-white">
      ...
      <h1 className="text-3xl font-bold mb-1 text-teal-600">小数カード</h1>
      ...
      <div className="w-full max-w-sm space-y-4 mb-8">
        ...
        {users.map(user => (
          <div key={user.id}
            className={`w-full flex items-center p-4 rounded-2xl shadow-sm border-2 transition-transform hover:-translate-y-0.5 ${user.color?.class ?? 'bg-teal-50'} ${user.color?.border ?? 'border-teal-200'}`}>
            ...
            <div className="flex items-center gap-2 shrink-0 ml-2">
              <button onClick={e => startEdit(user, e)}
                className="w-10 h-10 rounded-full bg-white/50 text-stone-500 hover:bg-teal-500 hover:text-white flex items-center justify-center transition-colors shadow-sm">
