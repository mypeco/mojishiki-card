...
export const LevelSelect = ({ user, settings, onUpdateSettings, badges, counts, onStart, onOpenRecord, onBack }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-gradient-to-b from-teal-50 to-sky-50 animate-fade-in overflow-hidden">
      {/* ヘッダー */}
      <header className="flex items-center gap-3 px-4 pt-4 pb-2 shrink-0">
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <span className="text-xl font-black text-slate-600 truncate">{user.name}、<span className="text-teal-500">レベルを選ぼう！</span></span>
        </div>
        <button onClick={() => onUpdateSettings({ ...settings, isSoundEnabled: !settings.isSoundEnabled })}
          className={`p-2 rounded-xl border-2 transition-all ${settings.isSoundEnabled ? 'bg-teal-50 border-teal-200 text-teal-600' : 'bg-slate-50 border-slate-100 text-slate-300'}`}>
      ...
      <div className="px-4 pb-2 shrink-0">
        <div className="max-w-md mx-auto flex items-center gap-2 bg-white rounded-2xl p-1.5 shadow-sm">
          ...
          {[['tenkey', '⌨️ テンキー', 'キーで答えを入力'], ['flash', '⚡ フラッシュ', 'タップでめくる']].map(([k, l, d]) => (
            <button key={k} onClick={() => onUpdateSettings({ ...settings, modeType: k })}
              className={`flex-1 py-2 rounded-xl font-bold text-sm transition-all flex flex-col items-center leading-tight
                ${modeType === k ? 'bg-teal-500 text-white shadow' : 'bg-slate-50 text-slate-400'}`}>
              <span>{l}</span>
              <span className={`text-[9px] font-bold ${modeType === k ? 'text-teal-100' : 'text-slate-300'}`}>{d}</span>
            </button>
          ))}
        </div>
      </div>
          ...
          {LEVELS.map(lv => {
            ...
            return (
              <button key={lv.id} onClick={() => onStart(lv.id)}
                className="bg-white rounded-2xl px-4 py-3 shadow-sm border-b-4 border-teal-100 active:border-b-0 active:translate-y-1 transition-all flex items-center gap-3 text-left">
                <div className="text-3xl shrink-0 w-10 text-center">{lv.icon}</div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-baseline gap-2">
                    <span className="text-[10px] font-black text-teal-400 shrink-0">{levelLabel(lv.id)}</span>
                ...
                <div className="shrink-0 flex flex-col items-end gap-0.5">
                  <span className="text-slate-500 font-black text-sm bg-teal-50 rounded-lg px-2 py-0.5 whitespace-nowrap">
                    {lv.example}
                  </span>
