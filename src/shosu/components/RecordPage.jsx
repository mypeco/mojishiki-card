...
export const RecordPage = ({ user, history, badges, onDelete, onDeleteAll, onBack }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-teal-50 animate-fade-in overflow-hidden">
      <header className="px-4 py-3 flex items-center justify-between border-b border-teal-100 bg-white shrink-0">
        <button onClick={onBack} className="text-slate-400 hover:text-slate-600">
          <ArrowLeftIcon className="w-6 h-6" />
        </button>
        <h1 className="text-lg font-black text-teal-600">📊 {user.name} の記録</h1>
              ...
              {LEVELS.map(lv => {
                ...
                return (
                  <div key={lv.id} className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-[10px] font-black text-teal-400 w-12 shrink-0">{levelLabel(lv.id)}</span>
